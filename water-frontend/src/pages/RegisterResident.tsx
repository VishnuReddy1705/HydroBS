...
import { UserPlus, Loader2, Droplet, User, Building, Mail, Lock } from "lucide-react"
      ...
      >
        ...
        <div className="glass-morphic bg-white/10 border border-white/20 rounded-[28px] p-8 md:p-10 shadow-2xl backdrop-blur-2xl">
          <div className="flex flex-col mb-6">
            <h2 className="text-2xl font-bold text-white">Join HydroBS</h2>
            <p className="text-sm text-slate-300">Create your resident utility dashboard account</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" autoComplete="off">
            ...
            <div className="space-y-1">
              <label htmlFor="flatNumber" className="block text-xs font-bold text-slate-300 tracking-wider uppercase pl-1">Flat No.</label>
              <div className="relative">
              ...
              {errors.flatNumber && <p className="text-xs text-rose-300 font-semibold pl-1">⚠️ {errors.flatNumber.message}</p>}
            </div>
          ...
          <p className="text-center text-xs text-slate-300 mt-6 font-medium">
            Already have a resident account? <Link to="/login" className="underline text-[#48CAE4] hover:text-[#00B4D8] font-bold">Log in</Link>
          </p>
        </div>
      </motion.div>
    </div>
